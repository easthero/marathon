...
<?php
$sexlist=array("男","女");
$form = new PFBC\Form("register");
$form->addElement(new PFBC\Element\Hidden("form", "register"));
$form->addElement(new PFBC\Element\HTML('<legend>报名表</legend>'));
$form->addElement(new PFBC\Element\Textbox("姓名:", "Name", array("required" => 1)));
//$MatchTypeOptions = array("半程马拉松", "十公里健康跑");
//$form->addElement(new PFBC\Element\Radio("赛事类型:", "MatchType", $MatchTypeOptions, array("required" => 1)));
...
$form->addElement(new PFBC\Element\Textbox("身份证号码:", "IDCard", array(
	"required" => 1,
	"validation" => new PFBC\Validation\RegExp("/^[0-9]{15}$|^[0-9]{18}$|^[0-9]{17}[xX]$/",
	"Error: 身份证号码不合法")
)));
$form->addElement(new PFBC\Element\Email("电子邮件:", "Email", array("required" => 1)));
$form->addElement(new PFBC\Element\Textbox("住址:", "Address", array("required" => 1)));
$RegionOptions = array("长沙", "株洲", "湘潭", "其它地区");
$form->addElement(new PFBC\Element\Radio("您所在的地区:", "Region", $RegionOptions, array("required" => 1)));
$form->addElement(new PFBC\Element\Textbox("紧急联系人:", "Contact", array("required" => 1)));
$form->addElement(new PFBC\Element\Number("紧急联系人电话:", "ContactPhone", array(
	"required" => 1,
	"validation" => new PFBC\Validation\RegExp("/^[0-9]{8,12}$/",
	"Error: 紧急联系人电话须为8至12位数字"),
)));
$Agreeoptions = array("本人已认真阅读了长株潭半程马拉松赛的《网上报名须知》及《参赛者声明》，并同意其中的各项条款！");
$form->addElement(new PFBC\Element\Checkbox(" ", "Agree", $Agreeoptions, array("required" => 1)));
